...
ENC_FILE * dec_file(FILE *fp, long len) {
    ...
    unsigned char buf[BUF_LEN], key[32];
    int i=0;
...
int encrypt_file(const char *fileIn, const char *fileOut) {
    ...
    aes_encrypt_ctx ctx[1];
    unsigned char iv[16]; /* initialisation vector */
    unsigned char inBuffer[BUF_LEN], outBuffer[BUF_LEN];
...
int set_enc_data(const char *file, ENC_FILE *ef) {
    int i;
    aes_encrypt_ctx ctx[1];
    unsigned char iv[16]; /* initialisation vector */
    unsigned char buf[BUF_LEN], buf_out[BUF_LEN];
    unsigned char key[32];
    int len = 0;
    int rlen = 0;
    FILE *fout = NULL;
    ...
    for(i = 0; i < 16; ++i)
        iv[i] = rand() & 0xFF;
    fwrite(enc_prefix, 1, strlen(enc_prefix), fout);
    fwrite(iv, 1, 16, fout);
    ...
    //     aes_ofb_crypt(buf, buf_out, i, iv, ctx);
    //     fwrite(buf_out, 1, i, fout);
    // }
    //
    ef->offset = 0;
    rlen = ef->len;
    while (rlen > 0) {
        ...
        memcpy(buf, ef->data+ef->offset, len);
        ef->offset += len;
            
        aes_ofb_crypt(buf, buf_out, len, iv, ctx);
        fwrite(buf_out, 1, len, fout);
